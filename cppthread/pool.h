...
public:
        ...
        , f_out(out)
    {
        if(pool_size == 0)
        {
            throw cppthread_out_of_range("the pool size must be a positive number (1 or more)");
        }
        if(pool_size > 1000)
        {
            throw cppthread_out_of_range("pool size too large (we accept up to 1000 at this time, which is already very very large!)");
        }
    ...
    W & get_worker(int i)
    {
        ...
        {
            throw cppthread_out_of_range("snap::thread::pool::get_worker() called with an index out of bounds.");
        }
    ...
    W const & get_worker(int i) const
    {
        if(static_cast<std::size_t>(i) >= f_workers.size())
        {
            throw cppthread_out_of_range("snap::thread::pool::get_worker() const called with an index out of bounds.");
        }
        return f_workers[i]->get_worker();
    }
